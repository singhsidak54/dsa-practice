...
public class PossibleBipartition {
    public static boolean visit(HashMap<Integer, Set<Integer>> adjacencyList, int index, int[] visited) {
        ...
        while (!queue.isEmpty()) {
            ...
            for(int neighbour : adjacencyList.get(curr)) {
                if(visited[curr] == visited[neighbour]) {
                    return false;
    ...
    public static boolean possibleBipartition(int n, int[][] dislikes) {
        ...
        for(int i=1; i<=n; i++) {
            if(visited[i] == 0) {
                if(!visit(adjacencyList, i, visited)) {
                    return false;
                }
            }
        }
